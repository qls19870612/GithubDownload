 ...
 */
public class IntValueIntHashMap {
     ...
     *   than zero.
     */
    public IntValueIntHashMap(int initialCapacity) {
        this(IntHashMap.calNotRehashCapacity(initialCapacity, 0.75f), 0.75f);
    }
     ...
     */
    public IntValueIntHashMap(int initialCapacity, float lf) {
        super();
    ...
    public String getMapInfo() {
        ...
        java.util.List<Integer> itemCount = new java.util.ArrayList<>();
        for (Entry e : table) {
    ...
    public int increment(int key, int amount) {
        if (amount <= 0) {
            throw new IllegalArgumentException("IntValueIntHashMap.increment��amount�������0: " + amount);
        }
        Entry tab[] = table;
        int index = getIndex(key);
        for (Entry e = tab[index]; e != null; e = e.next) {
            if (e.hash == key) {
                ...
                if (result <= 0) {
                    throw new IllegalStateException("IntValueIntHashMap.increment��, �������amount��, ����<=0��. �ӵ�amount: " + amount + ", �������֮��: " + result);
                }
        ...
        // Creates the new entry.
        Entry e = new Entry(key, amount, tab[index]);
        tab[index] = e;
        count++;
        return amount;
    }
     ...
     * @return ���ؼ���֮���value
     */
    public int decrement(int key, int amount) {
        if (amount <= 0) {
            throw new IllegalArgumentException("IntValueIntHashMap.decrement��amount�������0: " + amount);
        }
        Entry tab[] = table;
        int index = getIndex(key);
        for (Entry e = tab[index], prev = null; e != null; prev = e, e = e.next) {
            if (e.hash == key) {
                ...
                // �е����� < Ҫ�۵�, �״�
                throw new IllegalStateException("IntValueIntHashMapʱ, Ҫ�� " + amount + ", ��ԭ��ֻ�� " + e.value);
            }
        }
        // û�ҵ�, �״�
        throw new NoSuchElementException("IntValueIntHashMap.decrementʱ, keyû�ҵ�: " + key);
    }
    ...
    private abstract class HashIterator<E> implements ReusableIterator<E> {
        ...
        public void remove() {
            ...
            int k = current.hash;
            current = null;
            IntValueIntHashMap.this.removeEntryForKey(k);
        }
    ...
    //    }
    public ReusableIterator<Entry> newEntryIterator() {
