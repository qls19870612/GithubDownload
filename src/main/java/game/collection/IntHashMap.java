...
public class IntHashMap<T> {
    ...
    public IntHashMap(int initialCapacity) {
        this(calNotRehashCapacity(initialCapacity, 0.75f), 0.75f);
    }

    public static int calNotRehashCapacity(int initialCapacity, float loadFactor) {
        int a = (int) (initialCapacity / loadFactor);
        int b = (int) (a * loadFactor);

        if (initialCapacity > b) {
            a++;
            assert initialCapacity <= (int) (a * loadFactor);
        }
        return a;
    }
    ...
    void createEntry(int hash, T value) {
        ...
        table[bucketIndex] = new Entry<>(hash, value, e);
        count++;
    }
    ...
    public void putUnique(int key, T value) {
        ...
        if (old != null) {
            throw new IllegalArgumentException("id冲突! " + key + ", 已经有 " + old + ", 又要新加 " + value);
        }
    }

    public void putUnique(int key, T value, Object owner) {
        T old = putIfAbsent(key, value);
        if (old != null) {
            throw new IllegalArgumentException(owner + "id冲突! " + key);
        }
    }
