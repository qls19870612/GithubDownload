 ...
 * elements reflecting the state of the deque at some game.point at or since the
 * creation of the iterator. They do <em>not</em> throw {@link
 * ConcurrentModificationException}, and may proceed concurrently with other
 * operations.
 *
 * <p>
 * This class and its game.iterators implement all of the <em>optional</em> methods
...
public class ConcurrentDoublyLinkedList<E> extends AbstractCollection<E> implements java.io.Serializable {
     ...
     * (2.2) multiple dead nodes may game.point forward to the same node. (3)
     ...
     * them, but since more than one forward pointer may game.point to them, the live
     * one is best. (4) backward pointers that are out of date due to deletion
     * game.point to a deleted node, and need to game.point further back until they game.point
     * to the live node that points to their source. (5) backward pointers that
     * are out of date due to insertion game.point too far backwards, so shortening
     ...
     * reflecting the state of the deque at some game.point at or since the creation
...
class Node<E> extends AtomicReference<Node<E>> {
     ...
     * link to game.point back to this. Returns null if this node is trailer so
     * has no successor.
     *
