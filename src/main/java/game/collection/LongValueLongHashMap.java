...
/**
 * intΪkey��longΪvalue��hashmap�����̰߳�ȫ
 * @author java_1
 *
 */
public class LongValueLongHashMap {
    ...
    public static class Entry {
         ...
         */
        protected Entry(long key, int hash, long value, Entry next) {
            this.hash = hash;
            this.key = key;
            this.value = value;
            this.next = next;
     ...
     *   than zero.
     */
    public LongValueLongHashMap(int initialCapacity) {
     ...
     */
    public LongValueLongHashMap(int initialCapacity, float loadFactor) {
        super();
    ...
    public long increment(long key, long amount) {
        if (amount <= 0L) {
            ...
            for (e = tab[index]; e != null; e = e.next) {
                if (e.key == key) {
                    long result = e.value + amount;
                    if (result <= 0L) {
                        throw new IllegalStateException(
                                "IntValueLongHashMap.increment��, ������amount��, ����<=0��. �ӵ�amount: " + amount + ", ������֮��: " + result);
                    }
            ...
            e = new Entry(key, hash, amount, tab[index]);
            tab[index] = e;
            ++this.count;
            return amount;
        }
    ...
    public long decrement(long key, long amount) {
        if (amount <= 0L) {
            ...
            Entry[] tab = this.table;
            int index = this.getIndex(hashLong(key));
            Entry e = tab[index];
    ...
    private abstract class HashIterator<E> implements Iterator<E> {
        ...
        public void remove() {
            ...
            int k = current.hash;
            current = null;
            LongValueLongHashMap.this.removeEntryForKey(k);
        }
    ...
    private final class EntrySet extends AbstractSet<Entry> {
        ...
        @Override
        public void clear() {
            LongValueLongHashMap.this.clear();
        }
    }
