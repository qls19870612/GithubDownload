...
import game.collection.ByteArrayBuilder;
...
public class Utils {
    ...
    //            int count){
    //        randomRoundIndex(ThreadLocalRandom.current(), indexList, maxCount,
    //                count);
    //    }
    ...
    public static int max(IntArrayList values) {
        int maxValue = Integer.MIN_VALUE;
    ...
    public static int min(IntArrayList values) {
        int minValue = Integer.MAX_VALUE;
    ...
    public static int getMapSize(int estimateSize) {
        return ((int) (estimateSize / 0.75)) + 1;
    }

    public static IntHashSet[] createIntHashSetArr(int length) {
        IntHashSet[] arr = new IntHashSet[length];

        for (int i = 0; i < length; i++) {
            arr[i] = new IntHashSet();
        }
